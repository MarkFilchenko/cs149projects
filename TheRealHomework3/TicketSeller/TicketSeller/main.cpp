...
/**
 *
 * Model to represent a Customer/thread
 **/
class Customer
{
public:
...
private:
    int pricelevel;
    int id;
    int midjump;				//only used for mid seats
...
vector<Seller*>  highsellers, mediumsellers, lowersellers;
vector<Seat*> seats;
...
//check availability, if open seat return true. Else return false
    ...
    if (this->pricelevel == 0)	//all seats were checked, low price seller
    {
        for (int i = 0; i<100; i++)
        {
            Seat* seat = seats[i];
...
string Seat::getName()
{
    ...
    {
        ss << (seller->getName()).c_str()<< setfill('0') << setw(3) << customer->getID();
    }
    string = ss.str();
    return string;
}
...
int main()
{
    ...
    string string;
    vector<Customer*>* queue = highseller->getQueue();
    for(int i = 0; i<(queue->size()) ; i++)
    {
    ...
    for(int i = 0 ; i<3 ; i++)
    {
        ...
        vector<Customer*>* queue = mediumseller->getQueue();
    ...
    for(int i = 0 ; i<6; i++)
    {
        ...
        vector<Customer*>* queue = lowerseller->getQueue();
        for(int j = 0; j<(queue->size()) ; j++)
        {
